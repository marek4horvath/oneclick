...
class LogisticsProductInputsLengthOrderFilter extends AbstractFilter
{
    /**
     * @param mixed $value
     */
    protected function filterProperty(
        ...
        if ($property !== 'order' || !is_array($value) || !isset($value['productInputs.length']) || $resourceClass !== Logistics::class) {
            return;
        }
        ...
        $subQuery = $queryBuilder->getEntityManager()->createQueryBuilder()
            ...
            ->where("pi.logistics = {$rootAlias}")
            ->getDQL();

        $sortDirection = strtolower($value['productInputs.length']) === 'desc' ? 'DESC' : 'ASC';
