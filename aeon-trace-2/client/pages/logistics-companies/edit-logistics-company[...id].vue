...
definePageMeta({
    title: 'companies.editCompany',
    name: 'edit-logistics-company',
    layout: 'dashboard',
    middleware: 'auth',
    displayTitle: false,
})
    ...
    if (addressInput.value) {
        ...
        autocomplete.addListener('place_changed', () => {
            ...
            if (addressComponents) {
                ...
                const houseNo = addressComponents.find(component => component.types.includes('street_number'))?.long_name || ''
                const city = addressComponents.find(component => component.types.includes('sublocality'))?.long_name || ''
...
const submitHandler = async () => {
    ...
    try {
        const companyData = {
            ...
            typeOfTransport: form.value.typeTransports.map(id => `/api/transport_types/${id}`),
        }

        const response = await companiesStore.updateCompany(form.value.id, companyData)
        ...
        if (newImageFile.value) {
            const image = b64toBlob(newImageFile.value)
            const formData = new FormData()

            const imageType = mimedb[image.type]

            formData.append('file', image, `${form.value.id}.${imageType.extensions[0]}`)

            const companyLogo = await companiesStore.uploadCompanyLogo(form.value.id, formData)
        ...
        await router.push('/logistics-companies')
        $event('saveNewLogisticsCompany')
    } catch (error) {
        console.error('Failed to edit logistics company:', error)
    }
}
</script>
            ...
            >
                <div class="company-header">
                    <h1 style="color: #26A69A">
                        {{ t('logistics.editLogisticsCompany') }}
                    </h1>
                </div>
                        ...
                        <VRow style="margin-top: -60px">
                            ...
                            >
                                <VBtn
                                    ...
                                    @click="submitHandler"
                                >
                                    {{ t("logistics.editLogisticsCompanyCaps") }}
                                </VBtn>
                            </VCol>
                        </VRow>
                    </VForm>
