...
const productStepsStore = useProductsStore()
...
const fetchTraceData = async () => {
    const id = productStep.value.id

    try {
        const traceData = await productStepsStore.fetchProductInputMaterialsTrace(id)
        if (traceData) {
...
const fetchDpp = async () => {
    productStep.value = await productStepsStore.fetchProductStepByQrId(companySlug, qrId)

    if (productStep.value.productInputs.length === 0) {
        return
    }
    if (!inputs.value[productStep.value.id]) {
        inputs.value[productStep.value?.id] = {
            step: productStep.value,
            inputs: productStep.value?.productInputs,
        }
    }

    const coordinatesInputs = productStep.value?.productInputs.filter((input: any) => input.type === 'coordinates')
...
const downloadQr = () => {
    ...
    link.href = `${backendUrl.value}/media/step_qrs/${productStep.value?.qrImage}`
...
<template>
    ...
    >
        ...
        <header>
            ...
            <div class="qr-code">
                <VImg
                    :src="`${backendUrl}/media/step_qrs/${productStep.qrImage}`"
                    height="298"
                    max-height="298"
                    max-width="298"
        ...
        <div class="mb-5 summary">
            ...
            <div>
                <p class="font-weight-bold">
                    {{ $t('dpps.company') }}: {{ productStep?.company?.name }}
                </p>
                <p class="font-weight-bold">
                    {{ $t('dpps.createdBy') }}: {{ productStep?.userData?.firstName }} {{ productStep?.userData?.lastName }}
                </p>
                <p class="font-weight-bold">
                    {{ $t('dpps.createdAt') }}: {{ productStep?.createdAt }}
                </p>
            </div>
        </div>
    ...
    <PrintTemplate
        v-if="productStep"
        :qr-src="`${backendUrl}/media/step_qrs/${productStep?.qrImage}`"
        class="hidden"
    />
    <HistoryInput />
</template>
